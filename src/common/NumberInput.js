export default function NumberInput({ value, setValue, maxValue, minValue, className = "" }) {
  ...
  return <input type="number" value={value} min={minValue} max={maxValue} className={className}
...
function validateNumberInput(input, maxValue=Infinity, minValue=0){
  if(input === ""){
    return true;
  } else if(/^[0-9]+$/.test(input) && Number(input) <= maxValue && Number(input) >= minValue){
    return true;
  } else {
    return false;
  }
